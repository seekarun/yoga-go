...
const CREATE_FAILED_PHYSICAL_ID_MARKER = 'AWSCDK::CustomResourceProviderFramework::CREATE_FAILED',
  MISSING_PHYSICAL_ID_MARKER = 'AWSCDK::CustomResourceProviderFramework::MISSING_PHYSICAL_ID';
async function handler(event, context) {
  const sanitizedEvent = { ...event, ResponseURL: '...' };
  if (
    (exports.external.log(JSON.stringify(sanitizedEvent, void 0, 2)),
    event.RequestType === 'Delete' && event.PhysicalResourceId === CREATE_FAILED_PHYSICAL_ID_MARKER)
  ) {
  ...
  try {
    const userHandler = require(exports.external.userHandlerIndex).handler,
      result = await userHandler(sanitizedEvent, context),
      responseEvent = renderResponse(event, result);
    await submitResponse('SUCCESS', responseEvent);
  } catch (e) {
    const resp = { ...event, Reason: exports.external.includeStackTraces ? e.stack : e.message };
    (resp.PhysicalResourceId ||
      (event.RequestType === 'Create'
        ? (exports.external.log(
...
function renderResponse(cfnRequest, handlerResponse = {}) {
  ...
  return { ...cfnRequest, ...handlerResponse, PhysicalResourceId: physicalResourceId };
}
async function submitResponse(status, event) {
  ...
  await withRetries({ attempts: 5, sleep: 1e3 }, exports.external.sendHttpRequest)(
    req,
    responseBody
  );
}
