...
export async function getDomainStatus(domain: string): Promise<{
  ...
  const projectId = process.env.VERCEL_PROJECT_ID;
  if (!projectId) {
    return { exists: false, verified: false, error: 'Vercel integration not configured' };
  }
  ...
  try {
    ...
    return { exists: false, verified: false, error: 'Failed to connect to Vercel API' };
  }
}
...
export async function verifyDomain(domain: string): Promise<{
  ...
  const projectId = process.env.VERCEL_PROJECT_ID;
  if (!projectId) {
    return { success: false, verified: false, error: 'Vercel integration not configured' };
  }
