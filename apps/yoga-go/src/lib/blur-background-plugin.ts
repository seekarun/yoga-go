...
import type { HMSVideoPlugin, HMSPluginSupportResult } from '@100mslive/hms-video-store';
...
type HMSPluginUnsupportedTypesEnum = 'PLATFORM_NOT_SUPPORTED' | 'DEVICE_NOT_SUPPORTED';
...
declare global {
  interface Window {
    SelfieSegmentation: new (config: SelfieSegmentationConfig) => SelfieSegmentation;
  }
}
...
export class BlurBackgroundPlugin implements HMSVideoPlugin {
  ...
  private renderWithBlur(input: HTMLCanvasElement, results: SelfieSegmentationResults): void {
...
export async function getBlurPlugin(blurAmount = 10): Promise<BlurBackgroundPlugin | null> {
