...
export default function AdminInboxPage() {
  ...
  useEffect(() => {
    if (!authLoading && user && !isAdmin) {
      console.log('[DBG][admn/inbox] User is not admin, redirecting');
      router.push('/app');
    }
  ...
  const fetchEmails = useCallback(
    async (append = false) => {
      try {
        ...
        console.log('[DBG][admn/inbox] Fetching emails with params:', queryParams.toString());

        const response = await fetch(`/data/admn/inbox?${queryParams.toString()}`);
        ...
        console.error('[DBG][admn/inbox] Error fetching emails:', err);
  ...
  const handleEmailClick = async (email: Email) => {
    ...
    if (!email.isRead) {
      try {
        await fetch(`/data/admn/inbox/${email.id}`, {
          method: 'PATCH',
        ...
        console.log('[DBG][admn/inbox] Failed to mark as read:', err);
      }
    }
    router.push(`/admn/inbox/${email.id}`);
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p style={{ fontSize: '16px', opacity: 0.9 }}>
                Emails to hi@, contact@, privacy@myyoga.guru
              ...
              <div className="p-12 text-center">
                ...
                <p className="text-gray-500">
                  {filter === 'unread'
                    ...
                    : filter === 'starred'
                      ? 'No starred emails'
                      : 'No platform emails yet'}
                </p>
                <p className="text-sm text-gray-400 mt-1">
                  Emails to hi@, contact@, privacy@myyoga.guru will appear here
                </p>
              </div>
            ) : (
              emails.map(email => (
