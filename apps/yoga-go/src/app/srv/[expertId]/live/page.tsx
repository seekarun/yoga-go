...
export default function ExpertLiveDashboard({ params }: { params: Promise<{ expertId: string }> }) {
  ...
  const [_showInstantModal, _setShowInstantModal] = useState(false);
  const [_instantTitle, _setInstantTitle] = useState('');
  const [_instantDescription, _setInstantDescription] = useState('');
  const [_meetingLink, _setMeetingLink] = useState('');
  const [_meetingPlatform, _setMeetingPlatform] = useState<'zoom' | 'google-meet'>('google-meet');
  const [_creatingMeeting, _setCreatingMeeting] = useState(false);
  ...
  const _handleCreateInstantMeeting = async () => {
    if (!_meetingLink.trim()) {
      alert('Please provide a meeting link (Zoom, Google Meet, etc.)');
      return;
    }

    _setCreatingMeeting(true);
    try {
      ...
      const response = await fetch('/api/live/sessions/instant', {
        ...
        body: JSON.stringify({
          title: _instantTitle || 'Instant Meeting',
          description: _instantDescription || 'Join this instant meeting',
      ...
      _setCreatingMeeting(false);
    }
  };

  const _handleCopyToClipboard = (_text: string) => {
    navigator.clipboard.writeText(_text);
    alert('Copied to clipboard!');
  };

  const _handleCloseModal = () => {
    _setShowInstantModal(false);
    _setInstantTitle('');
    _setInstantDescription('');
    _setMeetingLink('');
    _setMeetingPlatform('google-meet');
