...
export default function AvailabilityManagementPage({
  ...
  const [recurringStartTime, _setRecurringStartTime] = useState('09:00');
  const [recurringEndTime, _setRecurringEndTime] = useState('17:00');

  // One-time form state
  const [oneTimeDate, setOneTimeDate] = useState('');
  const [oneTimeStartTime, _setOneTimeStartTime] = useState('09:00');
  const [oneTimeEndTime, _setOneTimeEndTime] = useState('17:00');
  ...
  const _handleRecurringSubmit = async () => {
  ...
  const _handleOneTimeSubmit = async () => {
    if (!oneTimeDate) {
  ...
  const handleDateClick = (value: Date | null | [Date | null, Date | null]) => {
  ...
  const _recurringSlots = availabilitySlots.filter(s => s.isRecurring);
  const _oneTimeSlots = availabilitySlots.filter(s => !s.isRecurring);
