...
export default function SurveyResponsesPage() {
  ...
  useEffect(() => {
    fetchResponses();
    // eslint-disable-next-line react-hooks/exhaustive-deps -- fetchResponses is stable, only runs when filters change
  }, [expertId, filterQuestion, filterAnswer]);
  ...
  const fetchResponses = async () => {
    try {
      ...
      const url = `/api/srv/experts/${expertId}/survey/responses${
      ...
      if (result.success) {
        ...
        setError(result.error?.message || 'Failed to load responses');
      }
    } catch (err) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        alert(`Error: ${result.error?.message || 'Failed to send email'}`);
      }
    } catch (err) {
