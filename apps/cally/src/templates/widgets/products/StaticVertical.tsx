...
export default function StaticVertical({
  ...
  const handleRemoveBg = useCallback(
    async (productId: string, imageUrl: string) => {
      ...
      setRemovingBg(true);
      try {
        const newUrl = await processRemoveBackground(
          imageUrl,
          "StaticVertical",
        );
        onCardStyleChange?.(productId, { bgRemovedImageUrl: newUrl });
      } catch (err) {
        console.error("[DBG][StaticVertical] Remove BG failed:", err);
      ...
      return (
        <div key={product.id} className={`${SCOPE}-card`}>
          <div
            ...
            className={`${SCOPE}-img-col${isImgSel ? ` ${SCOPE}-img-col--selected` : ""}`}
            onClick={
              isEditing
                ? () => {
                    setCardSel({ type: "image", productId: product.id });
                    setHeadingSelected(false);
                    setSubheadingSelected(false);
                  }
                : undefined
