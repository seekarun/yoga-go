 ...
 * Sent to attendees before a calendar event starts (24h and 10m reminders).
...
export type ReminderType = "24h" | "10m";
...
function getReminderLabel(reminderType: ReminderType): string {
  return reminderType === "24h" ? "24 hours" : "10 minutes";
}

function getHeaderColor(reminderType: ReminderType): string {
  return reminderType === "24h" ? "#6366f1" : "#f59e0b";
}

/**
 * Send an event reminder email to an attendee.
 * Errors are caught internally so they don't break the cron job.
 */
export async function sendEventReminderEmail(
  ...
  const headerColor = getHeaderColor(reminderType);

  try {
    console.log(
      `[DBG][eventReminderEmail] Sending ${reminderType} reminder to ${attendeeEmail} for "${eventTitle}"`,
    );

    const subject = `Reminder: ${eventTitle} — starting in ${label}`;
    ...
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          <!-- Header -->
          <tr>
            <td style="background: ${headerColor}; padding: 25px 30px; text-align: center;">
          ...
          <tr>
            <td style="padding: 30px;">
              ...
              <p style="font-size: 16px; color: #333; margin: 0 0 20px 0;">
                This is a reminder that <strong>${eventTitle}</strong> is starting in <strong>${label}</strong>.
              </p>

              <!-- Event Details -->
              <div style="background: #eef2ff; padding: 20px; border-radius: 8px; border-left: 4px solid ${headerColor}; margin-bottom: 25px;">
              ...
              <p style="font-size: 13px; color: #999; margin: 0; text-align: center;">
                This is an automated reminder from ${tenant.name}.
              </p>
            </td>
          </tr>
...
This is a reminder that ${eventTitle} is starting in ${label}.
...
${tenant.name}`;
    ...
    await emailClient.sendEmail({
      ...
      html,
      tags: [
        { Name: "EmailType", Value: `event-reminder-${reminderType}` },
        { Name: "TenantId", Value: tenant.id },
      ],
    });

    console.log(
      `[DBG][eventReminderEmail] ${reminderType} reminder sent to ${attendeeEmail}`,
    );
  } catch (error) {
    console.error(
      "[DBG][eventReminderEmail] Failed to send reminder email:",
      error,
    );
  }
}
