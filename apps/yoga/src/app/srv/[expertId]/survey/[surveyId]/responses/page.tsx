...
export default function SurveyResponsesPage() {
  ...
  // Filter out invalid responses (only show valid or pending validation)
  const validResponses = data.responses.filter(
    r => !r.validation || r.validation.status !== 'invalid'
  );
  const discardedCount = data.responses.length - validResponses.length;
  const filteredResponses = validResponses;
      ...
      <div className="px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-1">Valid Responses</h3>
            <p className="text-3xl font-bold text-gray-900">{filteredResponses.length}</p>
          </div>
          <div className="bg-white rounded-lg shadow p-6">
            <h3 className="text-sm font-medium text-gray-500 mb-1">Discarded</h3>
            <p className="text-3xl font-bold text-red-500">{discardedCount}</p>
            {discardedCount > 0 && (
              <p className="text-xs text-gray-400 mt-1">Invalid emails filtered out</p>
            )}
          </div>
          <div className="bg-white rounded-lg shadow p-6">
                      ...
                      <td className="px-6 py-4">
                        {response.metadata?.country ? (
                          <div className="flex items-center gap-1.5 text-sm text-gray-700">
                            ...
                            <span>{response.metadata.country}</span>
                          </div>
                        ) : (
                      ...
                      <td className="px-6 py-4">
                        ...
                        response.metadata.deviceType !== 'unknown' ? (
                          <div className="flex items-center gap-1.5 text-sm text-gray-600">
                            ...
                            <span>{response.metadata.deviceType}</span>
                          </div>
                        ) : (
                          <span className="text-gray-400 italic text-sm">-</span>
                        )}
