...
export default function AdminEmailDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading && user && !isAdmin) {
      console.log('[DBG][admn/inbox/detail] User is not admin, redirecting');
      router.push('/app');
    }
  ...
  const fetchEmail = useCallback(async () => {
    try {
      setLoading(true);
      console.log('[DBG][admn/inbox/detail] Fetching email:', emailId);

      const response = await fetch(`/data/admn/inbox/${emailId}`);
      ...
      console.error('[DBG][admn/inbox/detail] Error:', err);
  ...
  const handleDownloadAttachment = async (attachment: EmailAttachment) => {
    try {
      const response = await fetch(`/data/admn/inbox/${emailId}/attachments/${attachment.id}`);
      const data = await response.json();
      ...
      console.error('[DBG][admn/inbox/detail] Download error:', err);
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      const response = await fetch(`/data/admn/inbox/${emailId}/reply`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text: replyText,
      ...
      console.error('[DBG][admn/inbox/detail] Reply error:', err);
  ...
  return (
    <div style={{ paddingTop: '64px', minHeight: '100vh', background: '#f7fafc' }}>
      ...
      <div className="bg-white shadow">
        <div className="px-6 lg:px-8 py-4" style={{ maxWidth: '1400px', margin: '0 auto' }}>
          ...
          {showReply && (
            <div className="border-t border-gray-200 p-6">
              ...
              <div className="flex items-center justify-between mt-4">
                <p className="text-xs text-gray-500">Sending as hi@myyoga.guru</p>
