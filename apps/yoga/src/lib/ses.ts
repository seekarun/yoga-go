 ...
 */
export async function createDomainIdentity(domain: string): Promise<CreateDomainIdentityResult> {
  ...
  try {
    ...
    if ((error as { name?: string }).name === 'AlreadyExistsException') {
      console.log(`[DBG][ses] Domain ${domain} already exists in SES, fetching status`);
    ...
    console.error(`[DBG][ses] Failed to create SES identity for ${domain}:`, error);
    throw error;
  }
}
...
export async function getDomainVerificationStatus(
  ...
  try {
    ...
    const dkimStatus = (response.DkimAttributes?.Status || 'NOT_STARTED') as DkimStatus;
...
export async function deleteDomainIdentity(domain: string): Promise<void> {
  ...
  try {
    ...
    if ((error as { name?: string }).name === 'NotFoundException') {
      console.log(`[DBG][ses] Domain ${domain} not found in SES, nothing to delete`);
      return;
    }

    console.error(`[DBG][ses] Failed to delete SES identity for ${domain}:`, error);
    throw error;
  }
}
 ...
 */
export function getDnsRecordsForDomain(domain: string, dkimTokens: string[]): TenantDnsRecord[] {
  ...
  records.push({
    ...
    purpose: 'SPF - Authorizes Amazon SES to send emails on behalf of your domain',
  });
