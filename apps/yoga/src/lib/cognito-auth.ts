 ...
 * Direct Cognito API calls using AWS SDK for custom signup/login flows
 */
import {
...
console.log('[DBG][cognito-auth] Initializing Cognito client with region:', cognitoConfig.region);
console.log('[DBG][cognito-auth] User Pool ID:', cognitoConfig.userPoolId);
console.log('[DBG][cognito-auth] Client ID:', cognitoConfig.clientId?.substring(0, 10) + '...');
...
export async function signUp(params: SignUpParams): Promise<SignUpResult> {
  ...
  if (phone) {
    // Cognito requires phone in E.164 format (+1234567890)
 ...
 */
export async function confirmSignUp(params: ConfirmSignUpParams): Promise<ConfirmSignUpResult> {
 ...
 */
export async function resendConfirmationCode(params: ResendCodeParams): Promise<ResendCodeResult> {
  const { email } = params;
 ...
 */
export async function getUserInfo(params: GetUserInfoParams): Promise<CognitoUserInfo> {
