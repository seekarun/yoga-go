...
export default function CourseLessonsPage() {
  ...
  useEffect(() => {
    ...
    const pollInterval = setInterval(async () => {
      try {
        console.log('[DBG][lessons-page] Polling video status for:', pollingVideoId);
        ...
        if (data.success) {
          ...
          console.log('[DBG][lessons-page] Video status:', videoStatus, 'Ready:', isReady);
          ...
          if (formData.cloudflareVideoId === pollingVideoId) {
            ...
            if (isReady || videoStatus === 'error') {
              console.log('[DBG][lessons-page] Video processing complete, stopping poll');
          ...
          if (lessonToUpdate) {
            ...
            if (lessonToUpdate.cloudflareVideoStatus !== newStatus) {
              console.log('[DBG][lessons-page] Updating lesson status in DB');
        ...
        console.error('[DBG][lessons-page] Error polling video status:', err);
      }
  ...
  const fetchCourseAndLessons = async () => {
    try {
      setLoading(true);
      console.log('[DBG][lessons-page] Fetching course and lessons:', courseId);
      ...
      if (courseData.success) {
        ...
        console.log('[DBG][lessons-page] Course loaded:', courseData.data);
      } else {
      ...
      if (lessonsData.success) {
        ...
        console.log('[DBG][lessons-page] Lessons loaded:', lessonsData.data);
        ...
        if (processingLesson && !pollingVideoId) {
          console.log(
            '[DBG][lessons-page] Found processing video, starting poll:',
      ...
      console.error('[DBG][lessons-page] Error fetching data:', err);
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      console.log('[DBG][lessons-page] File selected:', file.name, file.size);
  ...
  const handleVideoUpload = async () => {
    ...
    try {
      console.log('[DBG][lessons-page] Starting video upload');
      ...
      console.log('[DBG][lessons-page] Got upload URL for video:', uid);
      ...
      xhr.upload.addEventListener('progress', e => {
        if (e.lengthComputable) {
          ...
          console.log('[DBG][lessons-page] Upload progress:', percentComplete, '%');
        }
      });
      ...
      console.log('[DBG][lessons-page] Video uploaded successfully:', uid);
      ...
      console.log('[DBG][lessons-page] Starting status polling for:', uid);
      setPollingVideoId(uid);

      alert('Video uploaded successfully! Processing status will update automatically.');
    } catch (err) {
      console.error('[DBG][lessons-page] Error uploading video:', err);
  ...
  const handleSaveLesson = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingLessonId) {
        ...
        console.log('[DBG][lessons-page] Lesson updated:', result.data);
      } else {
        // Create new lesson
        ...
        console.log('[DBG][lessons-page] Lesson created:', result.data);
      }
      ...
      console.error('[DBG][lessons-page] Error saving lesson:', err);
  ...
  const handleDeleteLesson = async (lessonId: string) => {
    ...
    try {
      ...
      console.log('[DBG][lessons-page] Lesson deleted:', lessonId);

      // Refresh lessons
      await fetchCourseAndLessons();
    } catch (err) {
      console.error('[DBG][lessons-page] Error deleting lesson:', err);
  ...
  const handlePublishCourse = async () => {
    ...
    try {
      ...
      console.log('[DBG][lessons-page] Course published successfully');

      // Redirect to expert dashboard
      router.push(`/srv/${expertId}`);
    } catch (err) {
      console.error('[DBG][lessons-page] Error publishing course:', err);
